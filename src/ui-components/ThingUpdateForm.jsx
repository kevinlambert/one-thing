...
export default function ThingUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = thingRecord
      ? { ...initialValues, ...thingRecord, Periods }
      : initialValues;
    setUserId(cleanValues.UserId);
    setText(cleanValues.text);
    setCreatedDateTime(cleanValues.createdDateTime);
    setUpdatedDateTime(cleanValues.updatedDateTime);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Thing, idProp)
        : thingModelProp;
      setThingRecord(record);
      const PeriodsRecord = record ? await record.Periods : undefined;
      setPeriods(PeriodsRecord);
    };
    queryData();
  }, [idProp, thingModelProp]);
  React.useEffect(resetStateValues, [thingRecord, Periods]);
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Thing.copyOf(thingRecord, (updated) => {
              Object.assign(updated, modelFields);
              if (!modelFields.Periods) {
                updated.thingPeriodsId = undefined;
              }
            })
          );
          if (onSuccess) {
